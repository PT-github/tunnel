...
*/
        ...
        methods: {
            ...
            getDeviceStyle (device) {   // 计算设备位置
                // 0左  1车道1  2车道2  3车道3  4车道4 ~~~~10右
                let top = 0, {laneNums, singleDoubleType} = this.tunnelInfo;
                ...
                if (singleDoubleType === 3) {   // 如果是双洞的话，设备位置还得根据左右洞位置调整
                    if (device.leftRightFlag == 1) {
                        top += tunnelLine + center; // 左洞要加上右洞+卷闸门的高度
                    } else if (device.leftRightFlag == 2) {
                        top = tunnelLine - top;     // 右洞的话设备位置从上到下应该是10~0，所以这里从上到下计算的高度要用整个洞的高度减掉，就可以调转过来了
                    }
                }
                return {
