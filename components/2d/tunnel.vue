  ...
  <div class='tunnel-container'>
    ...
    <div class="map">
      ...
      <div class="tunnel-2d">
        ...
        <!--类型是双洞，在中间放个间隔-->
        <div v-if="tunnelInfo.singleDoubleType===3" style="height: 60px;"></div>
        ...
        <template v-for="item in deviceList">
          <el-popover placement="top" trigger="hover">
            ...
            <section slot="reference"
                     ...
                     :style="[getDeviceStyle(item), { width: '15px', height: '15px', position: 'absolute' }]"
...
export default {
  ...
  computed: {
    ...
    tunnelStyle() {
      ...
      if (this.tunnelInfo.singleDoubleType !== 3) {
        ...
        // 获取最小桩号
        let beginMark = Math.min(startStakeMark, startStakeMarkRight);

        // 获取最大桩号
        let endMark = Math.max(endStakeMark, endStakeMarkRight);

        // 最大长度 Math.max(leftHoleLength, rightHoleLength);
        let maxHoleLength = endMark - beginMark;

        // 左洞长度
        leftHoleLength = endStakeMark - startStakeMark;
        // 右洞长度
        rightHoleLength = endStakeMarkRight - startStakeMarkRight;
        ...
        let maxLength = endMark - beginMark;
        let lengthV = 100 / maxHoleLength;
  ...
  methods: {
    ...
    getEmptyStyle(item) {
      ...
      return {
        position: 'absolute',
        top: singleDoubleType === 3 && leftRightFlag == 2 ? tunnelLine + center + 'px' : '0',
        height: tunnelLine + 'px',
    ...
    getDeviceStyle(device) {
      // 0左  1 车道1  2 车道2  3 车道3  4 车道4 ~~~~10右   -1 左外面  11 横洞   12 右外面   13 距离1  14 距离2  15 距离3
      let top = 0
      let {laneNums, singleDoubleType} = this.tunnelInfo;
      let {leftStakeMark, maxLength, sideHeight, padding, lineHeight, contentHeight, center, tunnelLine} = this.tunnelStyle;

      // console.log(device.orientationLocation)
      const test = padding + sideHeight + contentHeight + (laneNums - 1) * lineHeight + sideHeight / 2

      switch (device.orientationLocation) {
        case 0:
          // padding + height6/2
        ...
        case 11:
          // padding + 固定高度 + 线条数量*2 + height6/2
          // console.log(device, '11')
          // if (device.leftRightFlagName === '左洞') {
          //   console.log(234234234234)
          //   top = -20
          // }
          //
          // if (device.leftRightFlagName === '右洞') {
          //   top = 20
          // }
          // top = padding + sideHeight / 2 + 20;
          break
        case 12:
          console.log(device, '12')
          // top = test - 10;
          break
        case 13:
          console.log(device, '13')
          top = test + 20;
          break
        case 14:
          console.log(device, '14')
          top = test + 20;
          break
        default:
      ...
      // if (device.orientationLocation === 0) { // 左边
      //   top = padding + sideHeight / 2;   // padding + height6/2
      // } else if (device.orientationLocation === 10) { // 右边
      //   top = padding + sideHeight + contentHeight + (laneNums - 1) * lineHeight + sideHeight / 2;  // padding + 固定高度 + 线条数量*2 + height6/2
      // } else {
      //   top = padding + sideHeight + contentHeight / laneNums / 2 + (device.orientationLocation - 1) * (contentHeight / laneNums + 2);
      // }
      ...
      if (singleDoubleType === 3) {
        ...
        } else if (device.leftRightFlag == 2) {
      ...
      // function doorFlag() {
      //   if (device.orientationLocation === 11) {
      //     if ()
      //   }
      // }
