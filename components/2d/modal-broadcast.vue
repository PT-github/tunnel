    ...
    <div class="content flex">
        <div class="left">
            <p class="flex flex-space lefttitle">
                ...
                <uploadFiles busType="BroadCast" button-only @uploadOver="uploadOver"
                             single path></uploadFiles>
            </p>
            <ul class="actionlist scroll">
                <li class="flex flex-space" v-for="(item,index) in listSource" :key="index">
        ...
        <div class="right">
            ...
            <ul class="actionlist scroll">
                <li class="flex flex-space" v-for="(item,index) in deviceList" :key="index">
                         <span>{{item.deviceName}}<span class="status">
                            <i :class="item.deviceCommunicationsState===0?'oni':'offi'"></i>
...
<script>
    ...
    export default {
        ...
        methods: {
            uploadOver (file) {
                this.$service._2d.saveBroadcastSource({
            ...
            //播放
            confirm () {
                let deviceId = this.deviceList.filter(v => v.ischose).map(v => v.id).join(',');
                let tempId = this.listSource.filter(v => v.ischose).map(v => v.id).join(',');
                if (!deviceId || !tempId) {
                    return this.$message.warning('请选择设备和广播');
                }
                this.$service._2d.playBroadcast({
                    deviceId,
                    tempId,
                    tunnelId: this.tunnelId,
            ...
            stop () {
                ...
                this.$service._2d.stopBroadcast({
                    deviceId,
                    tunnelId: this.tunnelId
                }).then(() => {
                    this.$notifySuccess();
            ...
            // 获取设备列表
            getDeviceList () {
                this.$service._2d.getTunnelDeviceTypeList(this.tunnelId, 'broadcast').then(res => {
