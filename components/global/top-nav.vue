...
<!--        <img src="../../assets/images/top-nav/hngs-logo-1.png">-->
<!--        <h2>湖南高速隧道智能管控平台</h2>-->
<!--      </section>-->
...
export default {
  ...
  components: {HomeList},
  services: ['basic', 'user', 'home', 'index'],
  data() {
    return {
      loginInImg: '',
      tunnelName: '',
  ...
  mounted() {
    ...
    this.$service.index.getLoginLogoInfo({value: 'OneValueDictionary'}).then(res => {
      const loginInImg = res.find(item => item.name === 'LoginInImg')
      if (loginInImg) this.loginInImg = loginInImg.value
    })
  },
