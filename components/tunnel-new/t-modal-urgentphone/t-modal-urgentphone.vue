<!-- 电话 -->
<template>
  <view class="t-modal-urgentphone">
    <view class="header">
      <text>手动模式-紧急电话</text>
    </view>
    ...
    <view class="signallamp-content">
      <view class="search-container">
        <text class="location" :class="{active: leftRightFlag === 2}" @click="switchLeftRight(2)" v-if="singleDoubleType === 3 || singleDoubleType === 2">左洞</text>
        <text class="location" :class="{active: leftRightFlag === 1}" @click="switchLeftRight(1)" v-if="singleDoubleType === 3 || singleDoubleType === 1">右洞</text>
        <input class="input-dom" type="text" v-model="form.deviceName" @blur="getDeviceList" placeholder="设备名称或桩号" />
        <text class="status" :class="{active: form.workMode === 0}" @click="switchWorkMode(0)">空闲</text>
        <text class="status" :class="{active: form.workMode === 1}" @click="switchWorkMode(1)">呼叫中</text>
      ...
      <scroll-view :show-scrollbar="true" class="scroll-container" scroll-y="true">
        <view class="scroll-content">
          <view class="device-item" v-for="item in deviceList" :key="item.id" :class="{active: checkedList.indexOf(item.id) !== -1}" @click="handleSelect(item)">
            <view class="device-title" :class="{ active: item.workMode == 1 }" :style="getStyle(item)">
          ...
          <!-- <view class="device-item">
            ...
            <view class="status on"></view>
          </view> -->
        </view>
      </scroll-view>

      <!-- <view class="info-msg">
        <text>双击对应的紧急电话可进行呼叫</text>
      </view> -->
    </view>

    <view class="operation">
      <text class="cancel" @click="handleCancel">取消</text>
      <!-- <text class="start">执行</text> -->
    </view>
  </view>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        form: {
          ...
          tunnelId: null, // 隧道ID
          classifyNumber: 'urgentphone' // 分类
        },
        deviceList: [],
        checkedList: [], // 已选设备列表
        workMode: [], // 控制模式
        workModeChecked: null, // 已选控制模式
        deviceTypeCode: 'urgentphone'
      };
    },
    computed: {
      // 隧道单双洞
    ...
    methods: {
      ...
      handleSubmit () {
        ...
        this.$request.trafficLightsImplement({
          classifyNumber: 'urgentphone',
          deviceId: this.checkedList.join(','),
          workMode: this.workModeChecked
        }).then(res => {
          if (res.status === 1) {
            uni.showToast({
              title: '执行成功',
    ...
    .scroll-container {
      ...
      .scroll-content {
        ...
        .device-item {
          ...
          &:not(:nth-child(4n)) {
            margin-right: 7.0312rpx; //9.7656rpx; // 10.9375rpx; 28px
          }
          .status {
            position: absolute;
            top: 0;
