<template>
	<view class="file-outerBox" @touchmove.prevent v-if="isOpen">
		...
		<view class="file-address">
			...
			<scroll-view :show-scrollbar="true" :scroll-x="true" class="address-scroll">
		...
		<view class="">
			<scroll-view :show-scrollbar="true" :scroll-y="true" :style="{height: 'calc(100vh - ' +  (barHeight*2 + 280) + 'rpx)'}">
	...
	export default {
		...
		methods:{
          ...
          if (this.selectArr.length > 0) {
            ...
            // })
            // return
            let name = this.selectArr[0].name;
            for(let i = 0; i < this.fileArr.length; i++){
            	if(name == this.fileArr[i].name){
            		this.$set(this.fileArr[i],'select',false);
            		break;
            	}
            }
          }
