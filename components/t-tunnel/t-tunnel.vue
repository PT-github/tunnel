<template>
  <div class="t-tunnel">
    ...
    <!-- 左洞方向 -->
    <div class="center-lb" v-if="tunnelInfoData.singleDoubleType === 3 || tunnelInfoData.singleDoubleType === 2">
    ...
    <!-- 右洞方向 -->
    <div class="center-lb center-lb2"
    ...
    <!-- 2d背景图 -->
    <div :show-scrollbar="true" class="map" :class="{'map-noscroll': tunnelInfoData.showMode === 0}" ref="orderBox"
      ...
      <div class="tunnel-2d" ref="2d" id="2d" :style="tunnel2dStyle">
        ...
        <template v-if="tunnelInfoData.singleDoubleType === 3 || tunnelInfoData.singleDoubleType === 2">
          <div class="tunnel-wall" style="height: 50px" :style="{
                          width: tunnelStyle.leftWidth,
                          marginLeft: tunnelStyle.leftMargin,
                        }"></div>
          <div class="tunnel-line" :style="{
                          width: tunnelStyle.leftWidth,
                          marginLeft: tunnelStyle.leftMargin,
                        }">
            <div class="line-side"></div>
            <template v-for="(v, i) in tunnelInfoData.laneNums">
              <div class="line-road" v-if="i !== 0" :key="i+'w'" :style="{
        ...
        <template v-if="tunnelInfoData.singleDoubleType === 1">
          <div style="opacity:0.2;filter:alpha(opacity=20);">
            ...
            <div class="tunnel-line" :style="{
              ...
              <div class="line-side"></div>
              <template v-for="(v, i) in tunnelInfoData.laneNums">
                <div class="line-road" v-if="i !== 0" :key="i+'m'" :style="{
                                margin: `${tunnelStyle.contentHeight /
                                  tunnelInfoData.laneNums}px 0`,
                              }"><span>占位</span></div>
              </template>
        ...
        <template v-if="tunnelInfoData.singleDoubleType === 2">
          <div style="opacity:0.2;filter:alpha(opacity=20);">
            <div class="tunnel-line" :style="{
              ...
              <div class="line-side"></div>
              <template v-for="(v, i) in tunnelInfoData.laneNums">
                <div class="line-road" v-if="i !== 0" :key="i" :style="{
                                margin: `${tunnelStyle.contentHeight /
                                  tunnelInfoData.laneNums}px 0`,
                              }"><span>占位</span></div>
              </template>
        ...
        <template v-if="
          ...
          <div class="tunnel-line" :style="{
            ...
            <div class="line-side"></div>
            <template v-for="(v, i) in tunnelInfoData.laneNums">
              <div class="line-road" v-if="i !== 0" :key="i+'r'" :style="{
                              margin: `${tunnelStyle.contentHeight /
                                tunnelInfoData.laneNums}px 0`,
                            }"><span>占位</span></div>
            </template>
        ...
        <template v-for="(item, index) in tunnelDevices">
          <img class="icon" :key="item.id + '_' + index" :class="{ middle: item.orientationLocation === 11, [item.deviceTypeCode]: true }"
            :draggable="false" :style="getDeviceStyle(item)" @click.stop="[...showDetail(item)]"
            v-show="isShow(item)"
          ...
          <template v-if="item.sensorValTextList && item.sensorValTextList.length && isShow(item)">
            <div class="sensorValText" :key="item.id + '_sensorValText_' + index" :style="getDeviceStyle(item, 'text')" @click.stop="[...showDetail(item)]">
              <div class="sensorValTextContainer" :key="item.id + '_sensorValTextContainer_' + index">
                <div class="board-item" v-for="(text, idx) in item.sensorValTextList" :key="item.id + '_' + index + '_sensorValTextList_' + idx">
          ...
          <template v-if="item.deviceTypeCode === 'intelligenceboard' && item.content && item.content.length && isShow(item)">
            <div class="board" :key="item.id + '_board_' + index" :style="getDeviceStyle(item)" @click.stop="[...showDetail(item)]">
              <div class="border-swiper" :key="item.id + 'border-swiper_' + index">
                <div class="board-item" v-for="(board, idx) in item.content" :key="item.id + '_' + index + '_' + idx">
                  <span>{{ board }}</span>
                </div>
              </div>
            </div>
      ...
      <div v-if="
        ...
        <div class="iconTextLeft" v-for="(item, i) in tunnelInfoData.laneNums" :key="'iconTextLeft_' + i"
          :style="backleft(i, 2)">
      ...
      <div v-if="
        ...
        <div class="iconTextRight" v-for="(item, i) in tunnelInfoData.laneNums" :key="i+'s'" :style="backleft(i, 1)">
    ...
    <div class="tunnelBunntn" v-if="this.tunnelInfoData.showMode === 1">
      <span v-for="(item, index) in tunnelBunntn" :key="index"
        :class="{ 'sel-button': true, active: buttonClass === index }" @click="movebutton(index)">k{{ item }}
      </span>
    </div>
...
<style lang="scss" scoped>
  ...
  .center-lb {
    ...
    .center-lb-bg {
      width: 100%;
      height: 40px;
      background: url(../../assets/tunnel/lb.png) center center / 157px auto no-repeat;
  ...
  .map {
    ...
    .tunnel-wall {
      ...
      background-size: 5.8593rpx 50px;
      opacity: 0.7;
    }
